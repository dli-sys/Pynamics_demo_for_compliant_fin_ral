...
def Cal_robot(direction, given_l, angular_vel,start_angle, end_angle, ini_states, name1, system, video_on, x1,force_coeff):
    ...
    damping_r = 0
    tinitial = 0
    tfinal = (start_angle - end_angle) / angular_vel
    tstep = 1 / 30
    ...
    mO = Constant(100 / 1000, 'mO', system)
...
def cal_eff(x, x1, video_flag):
    ...
    # if error1 + error2 == 0:
        ...
        ini_states = numpy.array([0, 0, -pi/3, 0, 0, servo_speed])
        system1 = System()
        # In here we start calculate the front proposion
        drag_reduction_coef = 1
        fin_perp = 5.6
        fin_par = -0.1
        body_drag = 1
        video_on = video_flag
        given_l = 20
        start_angle, end_angle = [0, -pi / 2]
        angular_vel = pi / 10

        final1, states1, y1 = Cal_robot(direction, given_l, angular_vel, start_angle, end_angle, ini_states,'robot_p1.gif', system1, 1, [0, 0],[body_drag,fin_perp*drag_reduction_coef,fin_par*drag_reduction_coef])
        ...
        system2 = System()
        final2, states2, y2 = Cal_robot(-direction, given_l, angular_vel, start_angle, end_angle, final,'robot_p2.gif', system2, 1, [0, 0],[body_drag,fin_perp,fin_par])
